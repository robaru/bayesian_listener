...
behind :mod:`bayesian_listener`.  Equation numbers refer to [barumerli2023]_ and
[barumerli2026]_; the two papers use consistent notation.
...
1. **Feature extraction.**  The binaural stimulus generated by a source at
   ...
   noisy spatial features (Eq. 1 of [barumerli2023]_):
   ...
   uncertainty, with diagonal covariance (Eq. 2 of [barumerli2023]_):
...
2. **Bayesian inference.**  The listener's internal estimate of the source
   ...
   an elevation prior (Eqs. 4–6 of [barumerli2023]_):
...
[barumerli2026]_), it is approximated via Monte Carlo with :math:`M` samples
(Eq. 10 of [barumerli2026]_):
...
**Poor identifiability of** :math:`\sigma_\mathrm{prior}` **at large values.**
  ...
  positive bias at large ground-truth values (mean bias +19.9°, [barumerli2026]_).
...
[middlebrooks1999]_.  The gammatone filterbank used in
:func:`~bayesian_listener.utils.compute_features` follows the ERB-rate
scale of [glasberg1990]_.  The barycentric (VBAP) interpolation in
:func:`~bayesian_listener.utils.vbap_interpolate` follows
[pulkki1997]_, and parameter optimisation throughout
:mod:`bayesian_listener.fitting` is performed with BADS [acerbi2017]_.

.. rubric:: References

.. [barumerli2023] R. Barumerli, P. Majdak, M. Geronazzo, D. Meijer,
   F. Avanzini, and R. Baumgartner (2023).
   *A Bayesian model for human directional localization of broadband
   static sound sources.*
   Acta Acustica, 7, 12.
   https://doi.org/10.1051/aacus/2023006

.. [barumerli2026] R. Barumerli, F. Brinkmann, E. Zanoni, A. Hoyer,
   L. Picinali, and M. Geronazzo (2026).
   *Statistical validation and full-sphere extension of a Bayesian model
   for human static sound localisation.*
   Submitted to Acta Acustica.

.. [middlebrooks1999] J. C. Middlebrooks (1999).
   *Virtual localization improved by scaling nonindividualized
   external-ear transfer functions in frequency.*
   J. Acoust. Soc. Am., 106(3), 1493–1510.
   https://doi.org/10.1121/1.427147

.. [bau2022] D. Bau, J. M. Arend, and C. Pörschmann (2022).
   *Estimation of the Optimal Spherical Harmonics Order for the
   Interpolation of Head-Related Transfer Functions Sampled on Sparse
   Irregular Grids.*
   Frontiers in Signal Processing, 2, 884541.
   https://doi.org/10.3389/frsip.2022.884541

.. [benhur2019] Z. Ben-Hur, D. L. Alon, R. Mehra, and B. Rafaely (2019).
   *Loudness stability of binaural sound with spherical harmonic
   representation of sparse head-related transfer functions.*
   EURASIP J. Audio Speech Music Process., 2019(1), 5.
   https://doi.org/10.1186/s13636-019-0148-x

.. [glasberg1990] B. R. Glasberg and B. C. J. Moore (1990).
   *Derivation of auditory filter shapes from notched-noise data.*
   Hearing Research, 47(1–2), 103–138.
   https://doi.org/10.1016/0378-5955(90)90170-T

.. [pulkki1997] V. Pulkki (1997).
   *Virtual sound source positioning using vector base amplitude panning.*
   J. Audio Eng. Soc., 45(6), 456–466.

.. [acerbi2017] L. Acerbi and W. J. Ma (2017).
   *Practical Bayesian Optimization for Model Fitting with Bayesian Adaptive
   Direct Search.*
   Advances in Neural Information Processing Systems, 30.

.. [ahrens2012] J. Ahrens, M. R. P. Thomas, and I. J. Tashev (2012).
   *HRTF magnitude modeling using a non-regularized least-squares fit
   of spherical harmonics coefficients on incomplete data.*
   APSIPA Annual Summit and Conference, Hollywood, USA.
